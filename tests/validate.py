...
def validate_config() -> dict:
    ...
    # only validate version format, do not pin exact version
    version = cfg["version"]

    if not isinstance(version, str):
        fail("version must be string")

    if not re.match(r"^\d+\.\d+\.\d+$", version):
        fail("version must follow semantic version format")
...
if __name__ == "__main__":
    ...
    print(
        f"OK: yyb_go_addons {cfg['version']} static validation passed"
    )
